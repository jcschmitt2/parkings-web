#!/usr/bin/env python3
"""Intègre les tarifs validés depuis parkings_tarifs_paripark_a_verifier.xlsx → parkings.csv."""
...
def integrate(parkings_path: Path, excel_path: Path) -> int:
    ...
    for _, row in ok.iterrows():
        pid = clean(row.get("ID parking (base)"))
        if not pid:
            continue
        ...
        paripark_url = clean(row.get("URL Paripark"))
        if paripark_url:
            parkings.at[idx, "url_site"] = paripark_url

        if not clean(parkings.at[idx, "operateur"]):
            parkings.at[idx, "operateur"] = "PARIPARK"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Intègre tarifs Paripark validés dans parkings.csv")
    parser.add_argument("--parkings", default=str(DATA_DIR / "parkings.csv"))
    parser.add_argument("--input", default=str(WORK_DIR / "parkings_tarifs_paripark_a_verifier.xlsx"))
