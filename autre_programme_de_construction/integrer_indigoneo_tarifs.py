#!/usr/bin/env python3
"""Intègre les tarifs validés depuis parkings_tarifs_indigoneo_a_verifier.xlsx → parkings.csv."""
...
def integrate(parkings_path: Path, excel_path: Path) -> int:
    ...
    for _, row in ok.iterrows():
        ...
        indigo_url = clean(row.get("URL Indigo Neo"))
        if indigo_url:
            parkings.at[idx, "url_site"] = indigo_url
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Intègre tarifs Indigo Neo validés dans parkings.csv")
    parser.add_argument("--parkings", default=str(DATA_DIR / "parkings.csv"))
    parser.add_argument("--input", default=str(WORK_DIR / "parkings_tarifs_indigoneo_a_verifier.xlsx"))
