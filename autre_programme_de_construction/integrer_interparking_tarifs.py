#!/usr/bin/env python3
"""Intègre les tarifs validés depuis parkings_tarifs_interparking_a_verifier.xlsx → parkings.csv."""
...
CSV_SEP = ";"
INFO_PART = "interparking.fr"

EXCEL_TO_CSV = {
    "15 min (€)": "tarif_15mn_eur",
    "30 min (€)": "tarif_30mn_eur",
    "1 h (€)": "tarif_1h_eur",
    "1 h 30 (€)": "tarif_1h30_eur",
    "2 h (€)": "tarif_2h_eur",
    "3 h (€)": "tarif_3h_eur",
    "4 h (€)": "tarif_4h_eur",
    "24 h (€)": "tarif_24h_eur",
...
def integrate(parkings_path: Path, excel_path: Path) -> int:
    ...
    for _, row in ok.iterrows():
        ...
        ip_url = clean(row.get("URL Interparking"))
        if ip_url:
            parkings.at[idx, "url_site"] = ip_url
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Intègre tarifs Interparking validés dans parkings.csv")
    parser.add_argument("--parkings", default=str(DATA_DIR / "parkings.csv"))
    parser.add_argument("--input", default=str(WORK_DIR / "parkings_tarifs_interparking_a_verifier.xlsx"))
